...
//    std::cout << Qwb.coeffs().transpose() <<"\n"<<Qwb.toRotationMatrix() << std::endl;
    ...
    // points obs in image
    // 将world下feature points变换到相机坐标系下，根据相机只能看到前方的点判断能够观察到那些点
...
//                    (obs(1)*460 + 255) > 0 && ( obs(1)* 460 + 255) < params.image_w )
            {
                points_cam.push_back(points[i]);//相机看到的世界坐标系中的三维点
                features_cam.push_back(obs); //归一化平面上的feature点
            }
        }
